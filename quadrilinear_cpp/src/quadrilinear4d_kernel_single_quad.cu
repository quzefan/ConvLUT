...
__global__ void QuadriLinearForward(const int nthreads, const float* luts, const float* image1, const float* image2, const float* image3, const float* image4, float* output, const int dim, const int shift, const float binsize, const int width, const int height, const int batch) {
...
int QuadriLinearForwardLaucher(const float* luts, const float* image1, const float* image2, const float* image3, const float* image4, float* output, const int luts_dim, const int shift, const float binsize, const int width, const int height, const int batch) {
